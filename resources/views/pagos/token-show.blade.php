...
<div class="space-y-6">
  <section class="card p-6">
    <p class="text-xs uppercase tracking-widest text-gray-500">Orden de cobro</p>
    <h1 class="mt-2 text-2xl font-semibold text-gray-900">Folio {{ $pago->folio_unico ?: 'SIN FOLIO' }}</h1>
    <p class="mt-1 text-sm text-gray-600">Token: {{ $pago->token_publico }}</p>
  </section>
  ...
  <section class="card p-6">
    <div class="grid gap-4 md:grid-cols-2">
      <div>
        ...
        <p class="text-sm font-semibold text-gray-900">{{ $pago->paciente?->name ?? 'N/D' }}</p>
        <p class="text-xs text-gray-500">{{ $pago->paciente?->dni ?? 'N/D' }}</p>
      </div>
      <div>
        <p class="text-xs uppercase tracking-widest text-gray-500">Estado</p>
        <p class="text-sm font-semibold text-gray-900">{{ strtoupper((string) $pago->estado) }}</p>
      </div>
      <div>
        <p class="text-xs uppercase tracking-widest text-gray-500">Monto</p>
        <p class="text-sm font-semibold text-gray-900">{{ number_format((float) $pago->monto, 2) }} {{ $pago->moneda }}</p>
      </div>
      <div>
        <p class="text-xs uppercase tracking-widest text-gray-500">Metodo</p>
        <p class="text-sm font-semibold text-gray-900">{{ $pago->metodo_pago ? strtoupper((string) $pago->metodo_pago) : 'SIN DEFINIR' }}</p>
      </div>
      <div>
        <p class="text-xs uppercase tracking-widest text-gray-500">Cita</p>
        <p class="text-sm font-semibold text-gray-900">#{{ $pago->cita_id }}</p>
      ...
      <div>
        <p class="text-xs uppercase tracking-widest text-gray-500">Recibo</p>
        <p class="text-sm font-semibold text-gray-900">{{ $pago->receipt?->folio_recibo ?: 'No emitido' }}</p>
      </div>
    </div>
