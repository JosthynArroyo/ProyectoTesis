<aside class="dashboard-sidebar fixed top-0 bottom-0 left-0 z-40 h-screen -translate-x-full overflow-y-auto border-r border-gray-200/70 bg-white px-4 pb-6 pt-0 shadow-xl lg:translate-x-0 lg:shrink-0">
    ...
    <nav class="mt-7 flex flex-col gap-1.5 text-[0.95rem] font-semibold leading-6">
        <a href="{{ route('paciente.dashboard') }}" @class(['flex min-h-[44px] items-center gap-3 rounded-xl px-3 py-2.5 transition-colors', 'bg-gray-100 text-gray-900 font-medium' => request()->routeIs('paciente.dashboard'), 'text-gray-600 hover:bg-gray-50' => !request()->routeIs('paciente.dashboard')])>
        ...
        @if($bloqueoPagosPendientes)
            <div class="rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5 text-amber-800">
                ...
                <p class="mt-2 text-xs font-medium">Tienes órdenes de cobro vencidas de citas concluidas. Regulariza tu cuenta para agendar una nueva cita.</p>
            </div>
        @else
            <a href="{{ route('paciente.crear-cita') }}" @class(['flex min-h-[44px] items-center gap-3 rounded-xl px-3 py-2.5 transition-colors', 'bg-gray-100 text-gray-900 font-medium' => request()->routeIs('paciente.crear-cita', 'paciente.crear-cita.*'), 'text-gray-600 hover:bg-gray-50' => !request()->routeIs('paciente.crear-cita', 'paciente.crear-cita.*')])>
                <i class="ri-add-circle-line text-lg"></i> Agendar cita
            </a>
        @endif

        <a href="{{ route('paciente.citas') }}" @class(['flex min-h-[44px] items-center gap-3 rounded-xl px-3 py-2.5 transition-colors', 'bg-gray-100 text-gray-900 font-medium' => request()->routeIs('paciente.citas', 'paciente.citas.*', 'paciente.editar-cita'), 'text-gray-600 hover:bg-gray-50' => !request()->routeIs('paciente.citas', 'paciente.citas.*', 'paciente.editar-cita')])>
            <i class="ri-calendar-line text-lg"></i> Mis citas
        </a>

        <a href="{{ route('paciente.laboratorio.index') }}" @class(['flex min-h-[44px] items-center gap-3 rounded-xl px-3 py-2.5 transition-colors', 'bg-gray-100 text-gray-900 font-medium' => request()->routeIs('paciente.laboratorio.index', 'paciente.laboratorio.download'), 'text-gray-600 hover:bg-gray-50' => !request()->routeIs('paciente.laboratorio.index', 'paciente.laboratorio.download')])>
            <i class="ri-test-tube-line text-lg"></i> Resultados
        </a>

        <a href="{{ route('paciente.historial') }}" @class(['flex min-h-[44px] items-center gap-3 rounded-xl px-3 py-2.5 transition-colors', 'bg-gray-100 text-gray-900 font-medium' => request()->routeIs('paciente.historial*'), 'text-gray-600 hover:bg-gray-50' => !request()->routeIs('paciente.historial*')])>
