          ...
          <article class="card p-5">
            <div class="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p class="text-sm text-slate-600">Doctor: {{ optional($nota->cita->doctor)->name ?? 'Doctor/a' }}</p>
              </div>
              <div class="text-right">
                <div class="text-sm text-slate-600">{{ $nota->cita->fecha ? \Carbon\Carbon::parse($nota->cita->fecha)->format('d/m/Y') : '-' }} {{ $nota->cita->hora ? \Carbon\Carbon::parse($nota->cita->hora)->format('H:i') : '' }}</div>
