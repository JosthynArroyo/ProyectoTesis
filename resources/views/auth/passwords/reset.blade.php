    ...
    <div class="page-shell">
      <div class="card p-8">
        <div class="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p class="text-xs uppercase tracking-widest text-slate-500">Restablecer</p>
            <h1 class="mt-2 text-2xl font-semibold text-slate-900">Restablecer contraseña</h1>
            <p class="text-slate-600">Define tu nueva contraseña para continuar.</p>
          </div>
        </div>
        ...
        <form method="POST" action="{{ route('password.update') }}" class="mt-6 space-y-4">
          ...
          <div>
            ...
            @else
              <div class="text-xs text-slate-500">Usa el mismo correo con el que solicitaste el enlace.</div>
            @enderror
          </div>
          ...
          <div>
            ...
            @error('password')
              <div class="text-xs text-rose-600">{{ $message }}</div>
            @else
              <div class="text-xs text-slate-500">Mínimo 8 caracteres. Incluye letras, números y un carácter especial.</div>
            @enderror
          </div>
          ...
          <div>
            ...
            @error('password_confirmation')
              <div class="text-xs text-rose-600">{{ $message }}</div>
            @else
              <div class="text-xs text-slate-500">Debe coincidir con la nueva contraseña.</div>
            @enderror
          </div>
