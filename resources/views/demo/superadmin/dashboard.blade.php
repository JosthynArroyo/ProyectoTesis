...
@endphp
<div class="space-y-6" data-dashboard-page data-dashboard-role="superadmin">
    ...
    <div class="panel-action-bar panel-action-bar--between">
        ...
        <div class="panel-action-bar__actions">
            <button onclick="alert('Esta es una demo. La exportación de PDF está desactivada para no consultar datos reales.')" class="btn btn-outline btn-full-mobile">
    ...
    @if(($pendingPersonalizacion ?? 0) > 0)
        <x-ui.alert tone="warning" title="Personalizacion pendiente">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    Hay <strong>{{ $pendingPersonalizacion }}</strong> solicitud{{ $pendingPersonalizacion === 1 ? '' : 'es' }} pendiente{{ $pendingPersonalizacion === 1 ? '' : 's' }}.
                </div>
                <a class="btn btn-outline btn-sm" href="{{ route('demo.superadmin.solicitudes') }}">
                    Ver solicitudes
                </a>
            </div>
        </x-ui.alert>
    ...
    <section class="card p-5">
        <form method="GET" action="{{ route('demo.superadmin.dashboard') }}" class="flex flex-col gap-3 lg:flex-row lg:items-end lg:gap-4">
