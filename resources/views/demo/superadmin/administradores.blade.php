...
<div class="space-y-6">
  ...
  <form class="card p-5" method="GET" action="{{ route('demo.superadmin.administradores') }}">
      ...
      <table class="table users" role="region" aria-label="Listado de administradores">
        ...
        @forelse($admins as $admin)
          @php
            ...
            $isSusp = ($estado === 'suspended');
          @endphp
          <form id="delete-{{ $admin['id'] }}" action="{{ route('demo.superadmin.administradores') }}" method="GET">
            <input type="hidden" name="simulated_action" value="deleted">
            <input type="hidden" name="simulated_name" value="{{ $admin['name'] }}">
          </form>
          <form id="block-{{ $admin['id'] }}" action="{{ route('demo.superadmin.administradores') }}" method="GET">
            <input type="hidden" name="simulated_action" value="blocked">
            <input type="hidden" name="simulated_name" value="{{ $admin['name'] }}">
          </form>
          <form id="activate-{{ $admin['id'] }}" action="{{ route('demo.superadmin.administradores') }}" method="GET">
            <input type="hidden" name="simulated_action" value="activated">
            <input type="hidden" name="simulated_name" value="{{ $admin['name'] }}">
          </form>
          <form id="deactivate-{{ $admin['id'] }}" action="{{ route('demo.superadmin.administradores') }}" method="GET">
            <input type="hidden" name="simulated_action" value="deactivated">
            <input type="hidden" name="simulated_name" value="{{ $admin['name'] }}">
          </form>
          ...
          <tr data-user-row>
            ...
            <td data-label="Estado">
              <div class="space-y-1">
                ...
                @endif
                <span class="text-xs text-gray-500">Último acceso: {{ $admin['last_login_at'] ? $admin['last_login_at']->diffForHumans() : 'N/D' }}</span>
              </div>
            </td>
                ...
                <div class="relative">
                  ...
                  <div id="admin-actions-{{ $admin['id'] }}" class="kebab-menu" role="menu">
                    <button
                      ...
                      data-suspend-action="{{ route('demo.superadmin.administradores') }}"
                    >
  ...
  <div class="modal-dialog modal-dialog--sheet" role="document" tabindex="-1">
    <div class="card modal-sheet p-6">
      ...
      <form
        ...
        action="{{ route('demo.superadmin.administradores') }}"
      >
        <input type="hidden" name="simulated_action" value="suspended">
