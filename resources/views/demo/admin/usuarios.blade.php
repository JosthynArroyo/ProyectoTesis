...
@section('title','Usuarios | Administración - Demo')
@section('header-title','Usuarios')
@section('header-subtitle','Gestión y control de usuarios (Demo)')
...
@php
  ...
  $suspendTarget = old('user_id') ? $collection->firstWhere('id', (int) old('user_id')) : null;
@endphp
...
<div class="space-y-6">
  <div class="panel-action-bar">
    ...
    <a class="btn btn-primary btn-full-mobile" href="{{ route('demo.admin.usuarios.create', array_filter(['preset_role'=>request('role')])) }}">
  ...
  <form class="card p-5" method="GET" action="{{ route('demo.admin.usuarios.index') }}">
      ...
      <table class="table users" role="region" aria-label="Listado de usuarios">
        ...
        @forelse($users as $u)
          ...
          @unless($esAdmin || $isClinicalProfessional)
            <form id="delete-{{ $u->id }}" action="{{ route('demo.admin.usuarios.destroy', $u->id) }}" method="POST">@csrf @method('DELETE')</form>
          @endunless
          @unless($esAdmin)
            <form id="block-{{ $u->id }}" action="{{ route('demo.admin.usuarios.block', $u->id) }}" method="POST">
              @csrf
              @method('PATCH')
              <input type="hidden" name="reason" value="Bloqueo manual">
            </form>
            <form id="activate-{{ $u->id }}" action="{{ route('demo.admin.usuarios.activate', $u->id) }}" method="POST">
              @csrf
              @method('PATCH')
              <input type="hidden" name="reason" value="">
            </form>
            <form id="deactivate-{{ $u->id }}" action="{{ route('demo.admin.usuarios.deactivate', $u->id) }}" method="POST">
              @csrf
              @method('PATCH')
                ...
                <div class="table-actions">
                  <div class="relative">
                    ...
                    <div id="user-actions-{{ $u->id }}" class="kebab-menu" role="menu">
                      <a class="btn btn-ghost btn-sm justify-start" href="{{ route('demo.admin.usuarios.show',$u->id) }}" role="menuitem">
                        <i class="ri-eye-line"></i> Ver detalle
                      </a>
                      <a class="btn btn-ghost btn-sm justify-start" href="{{ route('demo.admin.usuarios.edit',$u->id) }}" role="menuitem">
                      ...
                      @unless($esAdmin)
                        <button
                          ...
                          data-suspend-action="{{ route('demo.admin.usuarios.suspend', $u->id) }}"
                        >
  ...
  <div class="modal-dialog modal-dialog--sheet" role="document" tabindex="-1">
    <div class="card modal-sheet p-6">
      ...
      <form
        ...
        action="{{ $suspendTarget ? route('demo.admin.usuarios.suspend', $suspendTarget->id) : '' }}"
      >
        @csrf
        @method('PATCH')
