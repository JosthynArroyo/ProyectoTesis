...
class UxAuditFixesTest extends TestCase
{
    ...
    public function test_face_login_modal_uses_single_state_surface_without_static_unregistered_copy(): void
    {
        $this->get(route('home.index'))
            ->assertOk()
            ->assertSee('data-face-state="initial"', false)
            ->assertSee('data-face-state-title', false)
            ->assertSee('data-face-password-tab', false)
            ->assertDontSee('No tienes rostro registrado')
            ->assertDontSee('Registrar reconocimiento facial');
    }

    public function test_face_login_returns_not_registered_state_when_no_faces_are_available(): void
    {
        $this->postJson(route('face.login'), [
    ...
    public function test_face_login_returns_not_recognized_state_when_descriptor_does_not_match(): void
    {
        ...
        $this->postJson(route('face.login'), [
            'descriptor' => array_fill(0, 128, 1.0),
        ])
            ->assertStatus(422)
            ->assertJson([
    ...
    public function test_face_login_returns_recognized_state_and_authenticates_matching_profile(): void
    {
        $user = User::factory()->create(['status' => User::STATUS_ACTIVE]);
